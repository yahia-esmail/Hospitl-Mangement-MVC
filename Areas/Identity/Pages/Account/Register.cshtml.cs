...
namespace Hospitl_Mangement_MVC.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<BaseEntity> _userStore;
        ...
        private BaseEntity CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BaseEntity>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BaseEntity)}'. " +
                    $"Ensure that '{nameof(BaseEntity)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BaseEntity> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BaseEntity>)_userStore;
        }
    }
}
